...
console.rule("[bold blue]Starting TiRex Benchmarking V2 Script...[/bold blue]")
...
class TirexBenchmarkerV2(FluxForecastingBenchmarkerV2):
    ...
    def run_pipeline(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # quantiles is [batch, prediction_length, n_quantiles] (still normalized)
...
def main() -> None:
    ...
    # --- Setup and Seeding ---
    console.rule("[bold yellow]TiRex Benchmarking V2 Setup[/bold yellow]")
    console.log(
        f"Model: [green]{MODEL}[/green] | Device: [blue]{TirexBenchmarkerV2.DEVICE}[/blue]"
    )
